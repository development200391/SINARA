...
[Route("api/v1/finance/customers")]
public sealed class CustomersController(AppDbContext dbContext) : FinanceControllerBase
{
    [HttpGet]
    public async Task<IActionResult> Get([FromQuery] CustomerPagedRequest request, CancellationToken ct)
    {
        ...
        return Ok(PagedResult<CustomerDto>.Create(items, totalCount, page, pageSize));
    }
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CustomerDto request, CancellationToken ct)
    {
        try
        {
            var normalizedCode = NormalizeRequired(request.Code, "Customer code is required.").ToUpperInvariant();
            ...
            var duplicate = await dbContext.FinCustomers
                .IgnoreQueryFilters()
            ...
            var entity = new FinCustomer
            {
                ...
                CreditLimit = decimal.Round(request.CreditLimit, 4, MidpointRounding.AwayFromZero),
            ...
            dbContext.FinCustomers.Add(entity);
            await dbContext.SaveChangesAsync(ct);

            var result = await dbContext.FinCustomers
                .AsNoTracking()
    ...
    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update(int id, [FromBody] CustomerDto request, CancellationToken ct)
    {
        try
        {
            var entity = await dbContext.FinCustomers.FirstOrDefaultAsync(x => x.Id == id, ct);
            if (entity is null)
            {
                return NotFound();
            }
            ...
            entity.CreditLimit = decimal.Round(request.CreditLimit, 4, MidpointRounding.AwayFromZero);
    ...
    public async Task<IActionResult> Delete(int id, CancellationToken ct)
    {
        ...
        var hasInvoices = await dbContext.FinArInvoices.AnyAsync(x => x.CustomerId == id, ct);
        var hasReceipts = await dbContext.FinArReceipts.AnyAsync(x => x.CustomerId == id, ct);
        if (hasInvoices || hasReceipts)
        {
            return BadRequest(new { message = "Customer already used in AR transactions and cannot be deleted." });
        }

        dbContext.FinCustomers.Remove(entity);
        await dbContext.SaveChangesAsync(ct);

        return NoContent();
    }

    private static readonly Expression<Func<FinCustomer, CustomerDto>> CustomerProjection = entity => new CustomerDto
    {
        Id = entity.Id,
        Code = entity.Code,
        Name = entity.Name,
